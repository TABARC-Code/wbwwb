...
(function (global) {
  ...
  function stableTrait(peep, salt) {
    ...
    var value = Math.abs(Math.floor((peep.x || 0) * 31 + (peep.y || 0) * 17 + type + salt * 101));
  ...
  function ShadowAudienceModel(options) {
    ...
    this.activeDuration = options.activeDuration || 9;
  ...
  ShadowAudienceModel.prototype.socialProof = function (peep, peeps) {
    ...
    for (var i = 0; i < peeps.length; i++) {
      ...
      nearby += 1;
      if (stateFor(peeps[i]).phase === "active") active += 1;
    }
  ...
  ShadowAudienceModel.prototype.advancePhases = function (peeps) {
    for (var i = 0; i < peeps.length; i++) {
      ...
      if (state.phase === "active") {
        state.activeAge += 1;
        if (state.activeAge >= this.activeDuration) {
          state.phase = "cooling";
  ...
  ShadowAudienceModel.prototype.spread = function (scene) {
    if (!scene || !scene.world) return;
    this.generation += 1;
    var peeps = scene.world.peeps.slice();
    this.advancePhases(peeps);
    var active = peeps.filter(function (peep) { return stateFor(peep).phase === "active"; });
    for (var i = 0; i < active.length; i++) {
      var sourceState = stateFor(active[i]);
      var infectiousness = pressure(sourceState) * Math.max(0.15, 1 - sourceState.activeAge / this.activeDuration);
      for (var j = 0; j < peeps.length; j++) {
        ...
        if (before === "susceptible") this.transmissions += 1;
      }
    }
    for (var p = 0; p < peeps.length; p++) this.assess(scene, peeps[p], peeps);
  };
  ShadowAudienceModel.prototype.maybeTransform = function (scene, peep) {
    if (!this.transform || !peep || !scene.world) return peep;
    var state = stateFor(peep);
  ...
  ShadowAudienceModel.prototype.summary = function (scene) {
    var result = { susceptible: 0, exposed: 0, active: 0, cooling: 0, transmissions: this.transmissions };
    if (!scene || !scene.world) return result;
    scene.world.peeps.forEach(function (peep) { result[stateFor(peep).phase] += 1; });
    return result;
  };
  ShadowAudienceModel.prototype.update = function (scene) {
    this.tick = (this.tick + 1) % 30;
    if (this.tick === 0) this.spread(scene);
  };
